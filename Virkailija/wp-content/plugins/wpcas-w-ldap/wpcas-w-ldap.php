		...
		if(!ldap_set_option($ds, LDAP_OPT_PROTOCOL_VERSION, 3)) {
			...
			if(!$bind) {
				$error = 'Anonymous bind to LDAP failed.';
                                ...
                                $search_group = ldap_search($ds, $wpcasldap_use_options['ldapbasedn'], "(&(objectClass=groupOfUniqueNames)(uniqueMember=uid=augkilp,ou=people,dc=example,dc=com)(cn=APP_VALINTAREKISTERI_CRUD))");
                                                            
				$info2 = ldap_get_entries($ds, $search_group);
