...
function get_currentuserinfo() {
    ...
    // Debuggausta varten logitetaan tieto milloin käyttäjä on kirjautunut / kirjautumaton - Anniina (ansal1)
...
if (isset($_REQUEST['logoutRequest'])) {
    ...
    $wpcasldap_sessionid = str_replace('.', '', $regs[1]);
    
    session_id($wpcasldap_sessionid);

    $sessionid = session_id($wpcasldap_sessionid);
    
    // Testataan tuhotaanko oikea sessio
    
    // error_log('CAS session ID: '.$sessionid); - Anniina (ansal1)
    
    session_destroy();
    
    //$sessionDestroy = session_destroy() ? 'true' : 'false';

    // error_log('Session ID destroyed: '.$sessionDestroy); - Anniina (ansal1)
...
function update_wpuser($wpuserID) {
    ...
    if(!$ds) {
        ...
        if(!ldap_set_option($ds, LDAP_OPT_PROTOCOL_VERSION, 3)) {
            ...
            if(!$bind) {
                ...
                $search = ldap_search($ds, $wpcasldap_use_options['ldapbasedn'], "uid=".$wp_username);
                ...
                $wp_mail = $wpuserdata->user_email;
                $wp_lastname = $wpuserdata->last_name;
                $wp_firstname = $wpuserdata->first_name;
                    
                if ($wp_mail == $ldap_mail) {
                    error_log('nothing updated');
                } else {
                    wp_update_user(array('ID' => $wpuserID, 'user_email' => $ldap_mail));
                    error_log('updated');
                }
                
                if ($wp_lastname == $ldap_sn) {
                    error_log('nothing updated');
                } else {
                    wp_update_user(array('ID' => $wpuserID, 'last_name' => $ldap_sn));
                    error_log('updated');
                }
                
                if ($wp_firstname == $ldap_cn) {
                    error_log('nothing updated');
                } else {
                    wp_update_user(array('ID' => $wpuserID, 'first_name' => $ldap_cn));
                    error_log('updated');
                }

            }
            ldap_close($ds);
            error_log('ldap yhteys suljettu');
        }
    }
    return;
}
...
/* $wpcasldap_use_options['userrole'] */

function ldapgroup($uid) {                                
    return 'subscriber';                          
}
